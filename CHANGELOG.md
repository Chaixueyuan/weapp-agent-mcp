...
- 修复连接建立并发、超时后资源清理、CLI 子进程生命周期、日志/默认项目跨进程持久化与连接目标隔离问题。
- 收紧参数 schema、断言/快照失败语义、函数执行与序列化边界，并让 `maxBytes` 真正约束最终文本结果。
- 修复 `mp_pollUntil` 整体超时预算、scenario 报告 Markdown 转义，以及 `switchTab` 错误接受 query 参数的问题。
- 文件截图现在校验目标必须是非空普通文件，并细分目录、只读文件系统、磁盘配额等本地输出错误。
- 修复触摸事件 `changedTouches` 兼容性、日志监听启用失败被吞掉，以及项目发现优先级与扫描边界问题。
...
说明：
- 为 `mp_screenshot` 增加 `timeoutMs` 参数，默认从 15 秒提升到 30 秒
- 截图执行前记录 route、模式、输出路径、连接方式等上下文
- 截图超时时返回更明确的诊断信息，并附带官方限制说明
- `mp_runScenario` / `mp_generateScenarioReport` 的 `screenshot` 步骤同步支持 `timeoutMs`
- 根据真实压测结论补充截图串行保护：截图调用走单通道串行执行，并在相邻截图之间加入短缓冲，避免并发调用打挂 automator / DevTools 截图链路
...
意义：
- 区分“工具层超时过短”和“底层截图能力/环境受限”
- 明确 `mp_screenshot` 当前应按串行能力使用，而不是并发能力
- 为后续继续排查截图失败原因提供更直接的上下文
...
当前项目已从“能连通基础 automator”推进到“更适合 agent 稳定调试”的状态，重点补的是：
- 连接与自动拉起稳定性
- `data` 读取超时保护
- 日志恢复与过滤
- 页面等待能力
...
### 4. README 改为面向当前项目的实际使用方式

相关提交：
- `618df0d` `docs: align README with local fork workflow`

核心内容：
- README 默认接入方式曾从“上游 npm 包”调整为“本地构建产物”
- MCP 配置示例改为直接指向本地 `dist/index.js`
- 权限示例改为与 MCP server key 一致的前缀（例如 `mcp__weapp-agent-mcp__...`）
- README 已纳入当前项目已有增强能力说明，如：
  - `mp_evaluate`
  - 本地构建使用方式
- 删除/修正了一些不再适合当前项目的旧表述
...
意义：
- 方便后续维护者或其他 agent 快速理解当前项目的推荐用法
...
- 该测试项目不是本仓库的一部分，但可作为当前项目的本机回归验证样本
...
建议：
- 若后续继续维护当前项目，可保留一个类似的本机测试小程序，专门覆盖导航、输入、列表、滚动、日志、截图等基础链路
