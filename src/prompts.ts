...
const connectionFlow = [
  ...
  "",
  "1. Call mp_ensureConnection. Its currentPage result is already the active page.",
  "2. Continue with mp_screenshot, page_*, or element_* tools.",
  "3. Treat mp_screenshot as a serialized single-lane capability; do not run screenshot steps in parallel.",
  "4. Prefer shorter segmented scenarios instead of one long high-pressure scenario.",
  "5. If an operation fails, call mp_healthCheck; call mp_recoverConnection only when needsRecovery=true.",
  "6. Use mp_diagnoseConnection only for read-only diagnosis after ensure/recovery fails or when explicitly requested.",
  "7. If a tool says PROJECT_SELECTION_REQUIRED, call mp_listProjects or provide projectSelection to mp_ensureConnection.",
  "8. Do not instruct cli open, cli auto, or cli quit, and do not switch ports automatically.",
].join("\n");
    ...
    },
    {
      ...
      load: async () => {
        return [
          ...
          "- If screenshot fails repeatedly, call mp_healthCheck; call mp_recoverConnection only when needsRecovery=true.",
